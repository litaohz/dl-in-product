...
for dir in `ls /data4/janusyang/KG/Anchor/WideAndDeep/tfrecords1201/`
  ...
  if [[ $dir = 20191[1-2][0-3][0-9] ]]; then
    if [ $dir -ge $min ]; then
      if [ $dir -le $max ]; then
        ...
        echo "start train: $traindir/tfrecords1201/$dir"
        #echo "start train: $traindir/tfrecords0812v2/$dir"
        echo "-------------------------"
        #此处训练是选择最后一天的作为测试--dev参数，之前的作为训练--train参数 
        source activate /data2/timmyqiu/env &&  python ./dw_train_weight.py --conf dw_conf_1201.ini --train $traindir/tfrecords1201/$dir/train/ --dev $traindir/tfrecords1201/$max/test/ 
      fi
    fi
  fi
done
