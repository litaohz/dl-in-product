  ...
  if [[ $dir = 201911[0-3][0-9] ]]; then
    if [ $dir -ge $min ]; then
      if [ $dir -le $max ]; then
        ...
        source activate /data2/timmyqiu/env &&  python ./dw_train_weight.py --conf dw_conf_1112.ini --train $traindir/tfrecords1112/$dir/train/ --dev $traindir/tfrecords1112/$max/test/ 
      fi
    fi
  fi
done
