...
def write_spec(config):
  ...
  train_dirs = config['input']['train'].split(',')
  ...
  train_inputs = input_fn(train_files, spec, shuffle=True, batch_size=500000)
  
  # get tfrecods
  num_epochs = 1
  logging.info("Start training for {} epoch(s)".format(num_epochs))
  def _gen(prefix, num):
    return ['{}_{}'.format(prefix,i) for i in range(1, num+1)]
  #keys = _gen('KC_50', 5) + _gen('KC_56', 8) + _gen('KC_57', 8)
  #keys += ['z_ctr_1', 'z_ctr_3', 'z_ctr_7', 'ctr_1', 'ctr_3', 'ctr_7']
