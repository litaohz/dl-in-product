...
def lbtw(loss_list):
  ...
  total_tensor = tf.get_variable("total",initializer=tf.constant(total,dtype=tf.int64))
  ...
  inv_rate_list = [tf.pow(l_hat_list[i], a) for i in range(task_num)]
  w_list = [tf.div(inv_rate_list[i], tf.add_n(inv_rate_list)) * task_num for i in range(task_num)]
  weight_loss = [tf.multiply(loss_list[i], w_list[i]) for i in range(task_num)]
  for i in range(task_num):
    tf.summary.scalar("lbtw/w_{}".format(i), tf.squeeze(w_list[i]))
  return weight_loss
...
def esmm_model_fn(features, labels, mode, params):
  ...
  if mode == tf.estimator.ModeKeys.PREDICT:
    ...
    weight_loss = lbtw([ctr_loss, ctcvr_loss])
