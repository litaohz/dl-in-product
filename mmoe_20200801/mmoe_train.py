...
def train(config , trainfile, testfile):
  ...
  for train_dir in train_dirs:
    for f in os.listdir(train_dir):
      if len(os.listdir(train_dir)) > 350:
        div = 125
      else:
        div = 50
      if f != "_SUCCESS":
  ...
  reg = float(config['model'].get('reg', '1e-5'))
  ...
  for i in range(epochs):
    ...
    for j in range(len(train_files)):
      model.train(input_fn=lambda: input_fn(train_files[j], spec, True, batch_size, mt=True), steps=max_steps)
